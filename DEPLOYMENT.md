...
| `SMS_API_URL` / `SMS_API_KEY` / `SMS_SENDER` | Yes (all three) | MeliPayamak SMS provider — powers the phone + OTP authentication flow (`RequestOtpUseCase` → `SmsProviderPort` → `RealSmsProviderAdapter`, see `ROJAN_SMS_Provider_Integration_Audit_v1.md`). No defaults, same fail-loud posture as `JWT_SECRET` — the app will not start under the `prod` profile without all three |
...
| `ROJAN_PUBLIC_BASE_URL` | No | Defaults to `https://rojanai.ir` (set in `docker-compose.prod.yml`) — the real customer-facing domain encoded into generated salon QR codes and staff-invite links |
...
- [ ] Set `SMS_API_URL`, `SMS_API_KEY`, `SMS_SENDER` from your MeliPayamak console (API Key integration — see `.env.example` for the exact field meanings and endpoint shape). `SMS_SENDER` must be a line MeliPayamak has already approved for this account, or sends will be rejected vendor-side.
- [ ] Confirm `.env` is not tracked by git (`git status` should not show it) — this applies to `SMS_API_KEY` exactly as much as `JWT_SECRET`/`DB_PASSWORD`
...
- [ ] Smoke-test the OTP flow against the real MeliPayamak endpoint with a real phone you control (see `ROJAN_MeliPayamak_SMS_Production_Wiring_Report_v1.md` §Live SMS Validation for the exact procedure) — confirm an SMS actually arrives, not just that the API call returns 200
