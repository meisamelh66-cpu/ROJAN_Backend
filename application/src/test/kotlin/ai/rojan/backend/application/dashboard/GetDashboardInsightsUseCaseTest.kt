...
class GetDashboardInsightsUseCaseTest {
    ...
    @Test
    fun `owner can fetch insights via explicit salonId`() {
        val ownerId = UserId.new()
        val salon = salonOwnedBy(ownerId)
    ...
    @Test
    fun `a MANAGER member can fetch insights via explicit salonId - RBAC, not ownerId equality`() {
        val ownerId = UserId.new()
    ...
    @Test
    fun `a caller with no relationship to the salon is denied`() {
        val ownerId = UserId.new()
        val salon = salonOwnedBy(ownerId)

        assertThrows(SalonAccessDeniedException::class.java) {
            useCase.execute(GetDashboardInsightsCommand(UserId.new(), salon.id))
        }
    }
}
