 ...
 * #3) - resolved here by reusing [BookingRepository.findByCustomerId]
 * (already existed, previously only powered the self-service
 * `GET /bookings/mine`) against the customer's linked [ai.rojan.backend.domain.user.UserId].
...
class GetCustomerBookingsUseCase(
    ...
    fun execute(command: GetCustomerBookingsCommand): PageResult<Booking> {
        ...
        return bookingRepository.findByCustomerId(userId, command.pageRequest, command.statusFilter, command.sortDirection)
    }
}
